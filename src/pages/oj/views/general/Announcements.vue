<template>
  <Panel shadow :padding="10" class="announcement-panel exclude">
    <div slot="title">
      {{ title }}
    </div>
    <div slot="extra">
...
export default {
  ...
  methods: {
    init() {
      this.isContest ? this.getContestAnnouncementList() : this.getAnnouncementList();
    },
  ...
  computed: {
    title() {
      return this.listVisible ? (this.isContest ? this.$i18n.t('m.Contest_Announcements') : this.$i18n.t('m.Announcements')) : this.announcement.title;
    },
    isContest() {
